...
Swal.fire({
  ...
  if (res) {
    ...
    function postFile(file) {
      ...
      if (end == 1) { console.log("Отправка окончание со значением 1"); }
          ...
          if (teacher == true) {
            ...
            for (var key in s) {
              if (key != "class&") {
                ...
                bt1.appendChild(document.createTextNode("Сонный"));
                var bt2 = document.createElement("button");
                bt2.setAttribute("id", "sbuttons");
                bt2.style.backgroundColor = color2;
                bt2.appendChild(document.createTextNode("Зевать"));
                var bt3 = document.createElement("button");
                bt3.setAttribute("id", "sbuttons");
                bt3.style.backgroundColor = color3;
                bt3.appendChild(document.createTextNode("Голова"));
            ...
            if (s[person]["drow"] == 1) {
              x1.appendChild(document.createTextNode("Сонный: " + "Сонный"));
            } else {
              x1.appendChild(document.createTextNode("Сонный: " + "Не сонный"));
            }
            if (s[person]["yawn"] == 1) {
              x2.appendChild(document.createTextNode("Зевать: " + "Зевающий"));
            } else {
              x2.appendChild(document.createTextNode("Зевать: " + "Не Зевающий"));
            }
            if (s[person]["pos"] == 0) {
              x3.appendChild(
                document.createTextNode("Позиция головы: " + "Смотрел на экран")
              );
            } else {
              x3.appendChild(
                document.createTextNode("Позиция головы: " + "Не смотрел")
              );
            }
            x4.appendChild(
              document.createTextNode("Номер: " + s[person]["number"])
            );
            x5.appendChild(
              document.createTextNode("Приостановлено: " + s[person]["paused"])
            );
            // console.log(x);
            live.innerHTML = "";
            live.appendChild(x1);
          ...
          setTimeout(function () {
            if (s['class&']['ClassEndTime'] == 0) {
              ...
              xhr.onload = function () {
                console.log(this.status);
                if (this.status == 200) {
                  console.log("работающий");
                  window.location.href = "http://localhost:5500/new"
                }
              }
              console.log("Сессия закончилась Учителем");
            }
          }, 50);
        } else {
          console.error(xhr);
    ...
    function startObjectDetection() {
      console.log("начиная обнаружения объекта");
    ...
    v.onloadedmetadata = () => {
      console.log("метаданные видео готовы");
      gotMetadata = true;
